...
7. IF a discovered Java_Source_File has a root-relative path that cannot be represented inside a node identifier — an empty path, a path containing a backslash, a leading separator, or a drive-letter prefix — THEN THE Parser_System SHALL record a Parse_Error identifying that path, SHALL skip only that file, and SHALL continue collecting the remaining Java_Source_Files. Such a path SHALL NOT be sanitized or re-encoded, because that would silently change the identity of the affected file.
8. THE Parser_System SHALL return every failure as a structured Parse_Error value from its public entry points and SHALL NOT propagate an exception across them; IF an unexpected internal failure occurs, THEN THE Parser_System SHALL convert it into a Parse_Error and SHALL write no Graph_Output_File.
