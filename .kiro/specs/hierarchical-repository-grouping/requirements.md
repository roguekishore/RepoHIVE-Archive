...
7. IF any node or edge in the provided Dependency_Graph violates the field types the shared JSON contract states — a node identifier that is not a non-empty string, a node kind outside the defined set, a non-string `directoryPath`, a non-string `packagePath` where present, a `file` node carrying `definedInFile`, a non-string edge endpoint, a frequency signal that is not a non-negative integer, a `strength` that is not a finite non-negative number where present, or an array element that is not an object — THEN THE Graph_Ingestor SHALL reject the input, perform no partial load, and return an error message identifying the offending element and field. THE Graph_Ingestor SHALL NOT coerce, clamp, or otherwise repair a violating value.
