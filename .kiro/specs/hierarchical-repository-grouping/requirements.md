...
7. IF any node or edge in the provided Dependency_Graph violates the field types the shared JSON contract states — a node identifier that is not a non-empty string, a non-string `directoryPath`, a non-string `packagePath` where present, a `file` node carrying `definedInFile`, a non-string edge endpoint, a frequency signal that is not a non-negative integer, a `strength` that is not a finite non-negative number where present, or an array element that is not an object — THEN THE Graph_Ingestor SHALL reject the input, perform no partial load, and return an error message identifying the offending element and field. THE Graph_Ingestor SHALL NOT coerce, clamp, or otherwise repair a violating value.
8. IF the provided Dependency_Graph contains a node whose kind is not one of `file`, `class`, or `function`, THEN THE Graph_Ingestor SHALL reject the input and return an error message identifying that node. The `group` and `repository` kinds are outputs of hierarchy construction, not inputs to it; accepting them produced an Index_File_Set whose `edges.json` referenced nodes absent from `nodes.json`.
9. IF the provided Dependency_Graph contains no node of kind `file`, THEN THE Graph_Ingestor SHALL reject the input and return an error message indicating that no file nodes were found, rather than emitting a Hierarchy consisting of a single childless repository node.
10. IF the provided Dependency_Graph contains two or more edges sharing the same ordered `(source, target)` pair, THEN THE Graph_Ingestor SHALL reject the input and return an error message identifying that pair. Edges in opposite directions between the same two nodes are distinct pairs and SHALL be accepted. THE Graph_Ingestor SHALL NOT fold duplicates into a single edge, since that would add and remove edges contrary to 1.4; summing signals between two entities into one edge is the producer's obligation.
...
10. THE Index_Serializer SHALL record, for each Group_Node produced from a Primary_Region, that Region's identifier and an ordinal distinguishing it from the Region's other Group_Nodes, such that the identifier-and-ordinal pair is unique across the Hierarchy; Group_Nodes that correspond to no Region — the Repository node and the intermediate Group_Nodes introduced to bound Repository fan-out — SHALL omit both. THE Index_Serializer SHALL record, on each per-Region construction decision in metadata.json, the identifiers of the Group_Nodes that decision produced. These fields are display-agnostic provenance: THE Grouping_System SHALL NOT emit a composed display label, and SHALL NOT derive any node identifier from them.
